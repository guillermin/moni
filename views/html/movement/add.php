<?php
$index = 0;
foreach ($movement->validate()->errors() as $error)
{
	echo $error;
}
echo Form::open('movement/add');
?>
<div id="info">
	<dl>
	...
	<ul>
		<?php foreach ($movement->get_transactions('incomes') as $income): ?>
			<li>
				...
				<span id="org_<?php echo $index; ?>_delete" class="deletex">x</span>
			</li>
			<?php $index ++; ?>
		<?php endforeach; ?>
		...
		<?php foreach ($movement->get_transactions('transfers') as $transfer): ?>
			<?php if ($transfer->amount > 0): ?>
				<li>
					...
					<span id="dst_<?php echo $index; ?>_delete" class="deletex">x</span>
				</li>
				<?php $index ++; ?>
			<?php endif; ?>
		<?php endforeach; ?>
...
<?php
echo Form::submit(NULL, 'Add movement');
echo Form::close();
?>
<div id="options">
	<div id="source">
